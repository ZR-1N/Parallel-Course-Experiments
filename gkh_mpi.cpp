...
namespace
{
    ...
    static void worker_loop_blocking(int m, int n, double *worker_compute_ms_sum, long long *tasks_done_sum)
    {
        while (true)
        {
            MPITaskHeader hdr{};
    ...
    Lab4MPIStats *stats)
{
    ...
    // 先广播全局规模，让 worker 知道要分配多大的局部 U/V
    int dims[2] = {0, 0};
    if (world_rank == 0)
    {
        dims[0] = B.rows();
    ...
    if (world_rank == 0)
    {
        ...
        for (int iter = 0; iter < opts.max_iter; ++iter)
        {
            ...
            while (active_workers > 0)
            {
                ...
                local_stats.merge_ms += (t1 - t0) * 1000.0;
    ...
    {
        worker_loop_blocking(m, n, &worker_compute_ms_sum, &worker_tasks_done_sum);
    }
